        ...
        <div class="mt-5">
            <form method="post" action="{{route('home-banner.update', $home_banner->id)}}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="banner_content" id="category" aria-describedby="emailHelp" value="{{old('banner_content') ?? $home_banner->content}}">
                </div>
                <div class="form-group">
                    <label for="category">Category Name</label>
                    <select name="product_id" class="form-control" >
                        @foreach($products as $product)
                            <option {{($home_banner->product_id == $product->id)?'selected':''}} value="{{$product->id}}">{{$product->name}}</option>
                        @endforeach
                    </select>

                </div>
                <div class="form-group">
                    <img type="file" src="{{old('image')?? Storage::url($home_banner->image)}}" class="img-thumbnail" id="image_preview" width="10%"  alt="">
                </div>
                <div class="form-group">
